...
def get_env_kwargs(env_name):
    ...
    elif env_name == 'MiniGrid-Empty-5x5-v0':
        def minigrid_empty_wrapper(env):
            return env
        kwargs = {
            'optimizer_spec': lander_optimizer(),
            'q_func': create_lander_q_network,
            ...
            'lander': True,
            'num_timesteps': 500000,
            'env_wrappers': minigrid_empty_wrapper
        }
        kwargs['exploration_schedule'] = lander_exploration_schedule(kwargs['num_timesteps']) 
...
class PairReplayBuffer(object):
    ...
    def store_frame(self, frame):
        ...
        """
        if self.obs is None:
            self.obs      = np.empty([self.size, 2] + list(frame[0].shape), dtype=np.float32 if self.lander else np.uint8)
            self.action   = np.empty([self.size, 2],                     dtype=np.int32)
            self.reward   = np.empty([self.size],                     dtype=np.float32)
            self.done     = np.empty([self.size],                     dtype=np.bool)
    ...
    def store_effect(self, idx, action, reward, done):
        ...
        reward: float
            ...
            True if episode was finished after performing that action.
        """
        self.action[idx][0] = action[0]
        self.action[idx][1] = action[1]
        self.reward[idx] = reward
        self.done[idx]   = done
