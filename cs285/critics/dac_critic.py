...
class DifferentialCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        for i in range(self.num_target_updates):
            V1_sp = self.forward(next_ob_no[:,0])
            V2_sp = self.forward(next_ob_no[:,1])
            target_val = (reward_n + (1 - terminal_n) * self.gamma * (V1_sp - V2_sp)).detach()
            for j in range(self.num_grad_steps_per_target_update):
                V1_s = self.forward(ob_no[:,0])
                V2_s = self.forward(ob_no[:,1])
                loss = self.loss(target_val, V1_s - V2_s)
                total_loss += loss
                self.optimizer.zero_grad()
                loss.backward()
                self.optimizer.step()

        # ob_no = torch.cat([ob_no[:,0], ob_no[:,1]], 1)
        # next_ob_no = torch.cat([next_ob_no[:,0], next_ob_no[:,1]], 1)
        
        # for i in range(self.num_target_updates):
        #     G_sp = self.forward(next_ob_no)
        #     target_val = (reward_n + (1 - terminal_n) * self.gamma * G_sp).detach()
        #     for j in range(self.num_grad_steps_per_target_update):
        #         G_s = self.forward(ob_no)
        #         loss = self.loss(target_val, G_s)
        #         total_loss += loss
        #         self.optimizer.zero_grad()
        #         loss.backward()
        #         self.optimizer.step()
